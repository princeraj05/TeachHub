...
export default function SyllabusTab({ subjectId, subjectName, onSyllabusUpdate }) {
  ...
  const fetchSyllabus = async () => {
    try {
      setLoading(true);
      const res = await axios.get(`${API}/api/syllabus/subject/${subjectId}`, {
  ...
  const handleToggleTopic = async (chapterId, topicId) => {
    try {
      await axios.patch(
        `${API}/api/syllabus/subject/${subjectId}/chapter/${chapterId}/topic/${topicId}`,
        {},
        { headers: { Authorization: `Bearer ${token}` } }
      );
      fetchSyllabus();
  ...
  if (loading) {
    return (
      <div className="py-20 text-center select-none flex flex-col items-center justify-center">
        ...
        <p className="text-slate-400 text-xs font-bold">Synchronizing {subjectName} syllabus...</p>
      </div>
    );
  }
      ...
      <div className="bg-white dark:bg-[#111827] border border-slate-200/50 dark:border-white/[0.05] p-5 rounded-2xl shadow-sm flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            <h3 className="text-sm font-black text-slate-900 dark:text-white uppercase tracking-wider">{subjectName} Curriculum & Syllabus</h3>
          ...
          <p className="text-[10px] text-slate-450 dark:text-slate-400 font-medium mt-1">
            Standard Admin curriculum pre-loaded. Teachers can update progress, add sub-topics, or insert custom modules.
          </p>
        </div>
