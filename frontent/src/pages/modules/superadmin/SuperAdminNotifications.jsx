  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] dark:bg-[#0B0F19] text-slate-900 dark:text-slate-100 p-4 md:p-8 font-sans transition-colors duration-200">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-slate-900 dark:text-white tracking-tight">Notifications Center</h1>
          <p className="text-sm text-slate-600 dark:text-slate-400 mt-1">Real-time system alerts, user requests, and platform notifications.</p>
        </div>
        <div className="flex items-center gap-3">
          <button
            onClick={fetchLiveNotifications}
            className="flex items-center gap-2 bg-white dark:bg-[#131B2E] hover:bg-slate-100 dark:hover:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 text-xs font-semibold px-3.5 py-2.5 rounded-xl transition-all"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <div className="bg-white dark:bg-[#131B2E] border border-slate-200 dark:border-slate-800 rounded-2xl p-5 flex items-center justify-between shadow-lg">
          <div>
            ...
            <h3 className="text-2xl font-extrabold text-amber-500 dark:text-amber-400 mt-1">{stats.approvals}</h3>
            <span className="text-xs text-amber-600 dark:text-amber-400/80 font-medium mt-1 inline-block">Requires review</span>
          </div>
        ...
        <div className="bg-white dark:bg-[#131B2E] border border-slate-200 dark:border-slate-800 rounded-2xl p-5 flex items-center justify-between shadow-lg">
          <div>
            <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Payments & Billing</p>
            <h3 className="text-2xl font-extrabold text-emerald-600 dark:text-emerald-400 mt-1">{stats.payments}</h3>
            <span className="text-xs text-emerald-600 dark:text-emerald-400/80 font-medium mt-1 inline-block">Transactions</span>
          </div>
        ...
        <div className="bg-white dark:bg-[#131B2E] border border-slate-200 dark:border-slate-800 rounded-2xl p-5 flex items-center justify-between shadow-lg">
          <div>
            <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Support Messages</p>
            <h3 className="text-2xl font-extrabold text-cyan-600 dark:text-cyan-400 mt-1">{stats.support}</h3>
            <span className="text-xs text-cyan-600 dark:text-cyan-400/80 font-medium mt-1 inline-block">Inquiries</span>
          </div>
        ...
        <div className="bg-white dark:bg-[#131B2E] border border-slate-200 dark:border-slate-800 rounded-2xl p-5 flex items-center justify-between shadow-lg">
          <div>
            <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">System Alerts</p>
            <h3 className="text-2xl font-extrabold text-blue-600 dark:text-blue-400 mt-1">{stats.system}</h3>
            <span className="text-xs text-blue-600 dark:text-blue-400/80 font-medium mt-1 inline-block">Platform logs</span>
          </div>
      ...
      <div className="bg-white dark:bg-[#131B2E] border border-slate-200 dark:border-slate-800 rounded-2xl p-4 mb-6 flex flex-col md:flex-row items-center justify-between gap-4">
            ...
            <button
              ...
              className={`px-4 py-2 rounded-xl text-xs font-bold transition-all whitespace-nowrap ${
                categoryFilter === cat
                  ...
                  : "bg-slate-100 dark:bg-[#0B0F19] text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white border border-slate-200 dark:border-slate-700/60"
              }`}
            >
        ...
        <div className="relative w-full md:w-80">
          ...
          <input
            ...
            className="w-full bg-slate-50 dark:bg-[#0B0F19] border border-slate-200 dark:border-slate-700/60 rounded-xl pl-10 pr-4 py-2 text-sm text-slate-800 dark:text-slate-200 focus:outline-none focus:border-blue-500 transition-colors"
          />
        </div>
      </div>
      ...
      <div className="space-y-3">
        {filteredNotifications.length === 0 ? (
          <div className="bg-white dark:bg-[#131B2E] border border-slate-200 dark:border-slate-800 rounded-2xl p-12 text-center text-slate-500 shadow-xl">
            <FaBell className="text-3xl mx-auto mb-3 text-slate-400 dark:text-slate-600" />
          ...
          filteredNotifications.map((item) => {
            ...
            return (
              <div
                key={item.id}
                className={`bg-white dark:bg-[#131B2E] border rounded-2xl p-4 md:p-5 transition-all flex flex-col md:flex-row items-start md:items-center justify-between gap-4 shadow-lg ${
                  item.unread ? "border-blue-500/40 bg-blue-50/20 dark:bg-[#131B2E]" : "border-slate-200 dark:border-slate-800/80 opacity-90 dark:opacity-80"
                }`}
              >
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <div className="flex items-center gap-2 flex-wrap">
                      <h4 className="text-sm md:text-base font-bold text-slate-900 dark:text-white tracking-tight">{item.title}</h4>
                      {item.unread && (
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 text-[10px] font-extrabold rounded-full bg-amber-500/20 text-amber-600 dark:text-amber-400 border border-amber-500/30">
                          <span className="w-1.5 h-1.5 rounded-full bg-amber-500 animate-pulse"></span>
                          Unread
                        </span>
                      )}
                    </div>
                    <p className="text-xs text-slate-600 dark:text-slate-400 mt-1 max-w-2xl leading-relaxed">{item.description}</p>
                ...
                <div className="flex items-center gap-2 self-end md:self-center flex-shrink-0">
                  <button
                    ...
                    className="px-3 py-1.5 rounded-xl text-xs font-semibold bg-slate-100 dark:bg-[#0B0F19] text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white border border-slate-200 dark:border-slate-700 hover:border-slate-300 dark:hover:border-slate-600 transition-colors flex items-center gap-1.5"
                  >
                  ...
                  <button
                    ...
                    className="p-2 text-slate-400 hover:text-blue-500 dark:hover:text-blue-400 bg-slate-100 dark:bg-[#0B0F19] border border-slate-200 dark:border-slate-700/60 rounded-xl transition-colors"
                  >
                    <FaCheckCircle className={item.unread ? "text-slate-400 dark:text-slate-500" : "text-blue-500 dark:text-blue-400"} />
                  </button>

                  <button
                    onClick={() => handleDelete(item.id)}
                    title="Dismiss Notification"
                    className="p-2 text-slate-400 hover:text-rose-500 dark:hover:text-rose-400 bg-slate-100 dark:bg-[#0B0F19] border border-slate-200 dark:border-slate-700/60 rounded-xl transition-colors"
                  >
                    <FaTrash />
                  </button>
                </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white dark:bg-[#131B2E] border border-slate-200 dark:border-slate-800 rounded-2xl w-full max-w-md p-6 shadow-2xl">
            <div className="flex items-center justify-between mb-4 border-b border-slate-200 dark:border-slate-800 pb-3">
              ...
              <button onClick={() => setViewItem(null)} className="text-slate-400 hover:text-slate-700 dark:hover:text-white"><FaTimes /></button>
            </div>
            ...
            <div className="space-y-3">
              ...
              <h4 className="text-base font-bold text-slate-900 dark:text-white">{viewItem.title}</h4>
              <p className="text-xs text-slate-700 dark:text-slate-300 leading-relaxed bg-slate-50 dark:bg-[#0B0F19] p-3.5 rounded-xl border border-slate-200 dark:border-slate-800">
              ...
              <div className="flex justify-between items-center text-xs text-slate-500 dark:text-slate-400 pt-2">
                ...
                <span className={viewItem.unread ? "text-amber-600 dark:text-amber-400 font-semibold" : "text-slate-500"}>
            ...
            <div className="mt-6 flex justify-end">
              <button onClick={() => setViewItem(null)} className="px-4 py-2 rounded-xl text-sm font-semibold bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200 hover:bg-slate-200 dark:hover:bg-slate-700">Close</button>
            </div>
          </div>
        </div>
      )}
