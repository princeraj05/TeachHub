...
function StudentAttendance() {
  ...
  return (
    <div style={{ fontFamily: SORA }} className="w-full max-w-5xl mx-auto space-y-6 text-left select-none pb-10 px-2 sm:px-4">
      ...
      {/* Overall KPI Cards grid row */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        
        {/* Card 1: Total Sessions */}
        <div className="bg-white dark:bg-[#0B132A] border border-slate-200/80 dark:border-white/[0.08] rounded-2xl sm:rounded-3xl p-4 sm:p-5 shadow-sm">
          <div className="w-9 h-9 rounded-xl bg-purple-500/10 text-[#7C3AED] border border-[#7C3AED]/25 flex items-center justify-center mb-3">
            <FaCalendarAlt className="text-sm" />
          </div>
          <p className="text-2xl sm:text-3xl font-black text-slate-900 dark:text-white tracking-tight mb-1">{stats.total}</p>
          <div className="flex justify-between items-center text-[10px] text-slate-400 dark:text-slate-500 font-extrabold uppercase tracking-wider">
            <span>TOTAL SESSIONS</span>
            <span className="text-[9px] text-[#7C3AED] dark:text-[#A78BFA] lowercase font-bold">Live</span>
          </div>
        </div>

        {/* Card 2: Present */}
        <div className="bg-white dark:bg-[#0B132A] border border-slate-200/80 dark:border-white/[0.08] rounded-2xl sm:rounded-3xl p-4 sm:p-5 shadow-sm">
          <div className="w-9 h-9 rounded-xl bg-emerald-500/10 text-emerald-500 border border-emerald-500/25 flex items-center justify-center mb-3">
            <FaCheckCircle className="text-sm" />
          </div>
          <p className="text-2xl sm:text-3xl font-black text-slate-900 dark:text-white tracking-tight mb-1">{stats.present}</p>
          <div className="flex justify-between items-center text-[10px] text-slate-400 dark:text-slate-500 font-extrabold uppercase tracking-wider">
            <span>PRESENT</span>
            <span className="text-[10px] text-emerald-500 font-extrabold">{stats.presentRate}%</span>
          </div>
        </div>

        {/* Card 3: Absent */}
        <div className="bg-white dark:bg-[#0B132A] border border-slate-200/80 dark:border-white/[0.08] rounded-2xl sm:rounded-3xl p-4 sm:p-5 shadow-sm">
          <div className="w-9 h-9 rounded-xl bg-rose-500/10 text-rose-500 border border-rose-500/25 flex items-center justify-center mb-3">
            <FaTimesCircle className="text-sm" />
          </div>
          <p className="text-2xl sm:text-3xl font-black text-slate-900 dark:text-white tracking-tight mb-1">{stats.absent}</p>
          <div className="flex justify-between items-center text-[10px] text-slate-400 dark:text-slate-500 font-extrabold uppercase tracking-wider">
            <span>ABSENT</span>
            <span className="text-[10px] text-rose-500 font-extrabold">{stats.absentRate}%</span>
          </div>
        </div>

        {/* Card 4: Attendance Rate */}
        <div className="bg-white dark:bg-[#0B132A] border border-slate-200/80 dark:border-white/[0.08] rounded-2xl sm:rounded-3xl p-4 sm:p-5 shadow-sm">
          <div className="w-9 h-9 rounded-xl bg-blue-500/10 text-blue-500 border border-blue-500/25 flex items-center justify-center mb-3">
            <FaChartPie className="text-sm" />
          </div>
          <p className="text-2xl sm:text-3xl font-black text-slate-900 dark:text-white tracking-tight mb-1">{stats.presentRate}%</p>
          <div className="flex justify-between items-center text-[10px] text-slate-400 dark:text-slate-500 font-extrabold uppercase tracking-wider">
            <span>RATE</span>
            <span className="text-[9px] inline-flex items-center gap-1 text-emerald-500 font-extrabold">
      ...
      <div className="bg-white dark:bg-[#0B132A] border border-slate-200/80 dark:border-white/[0.08] rounded-3xl p-5 sm:p-6 shadow-sm">
        <div className="flex items-center justify-between gap-3 mb-6 pb-4 border-b border-slate-100 dark:border-white/5">
          ...
          <div className="shrink-0 bg-slate-50 dark:bg-[#0B132A]/40 border border-slate-200/80 dark:border-white/[0.06] text-slate-600 dark:text-slate-300 px-3 py-1.5 rounded-xl text-xs font-black flex items-center gap-1 shadow-sm">
            <span>This Month</span>
            <span className="text-[10px] text-slate-400">▼</span>
          </div>
        </div>

        {/* Double charts split grid (circle on left, weekly bar chart on right) */}
        <div className="grid grid-cols-1 md:grid-cols-5 gap-6 items-center">
          
          {/* Left Column: Radial progress ring + Legend checklist */}
          <div className="md:col-span-2 flex flex-row items-center gap-5 justify-center md:justify-start border-r-0 md:border-r border-slate-100 dark:border-white/5 pr-0 md:pr-4">
            ...
            <div className="relative shrink-0 flex items-center justify-center select-none w-28 h-28 sm:w-32 sm:h-32">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center text-center">
                ...
                <span className="text-[8px] text-slate-400 dark:text-slate-500 font-extrabold uppercase tracking-wide">
                  Overall
                </span>
              </div>
            </div>
            ...
            {/* Legend Stats list */}
            <div className="space-y-1.5 text-[11px] text-slate-600 dark:text-slate-300 font-black">
            ...
            <div className="flex-1 flex items-end justify-around h-full pl-2">
              {[
                ...
                <div key={week.name} className="flex flex-col items-center flex-1 h-full justify-end">
                  <div
                    className="w-5 bg-emerald-500 rounded-t-md hover:bg-emerald-600 transition-all duration-300 relative group cursor-pointer"
