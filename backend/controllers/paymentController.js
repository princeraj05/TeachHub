...
const broadcastEvent = (req, eventName, payload = {}) => {
  try {
...
async function finish(orderId, paymentId, signature = "", req = null) {
  ...
  if (!["Pending", "Processing"].includes(payment.status)) throw Object.assign(new Error("Payment cannot be finalized"), { status: 409 });
  const c = await getSettingsForReceiver(payment.receiver, payment.purpose, payment.schoolName);
  const remote = await gateway("GET", `/v1/payments/${encodeURIComponent(paymentId)}`, null, c);
  if (remote.order_id !== orderId || remote.amount !== payment.amount || remote.currency !== payment.currency || remote.status !== "captured") throw invalid("Gateway payment verification failed");
  const paidAt = new Date();
  const updated = await Payment.findOneAndUpdate({ _id: payment._id, status: { $in: ["Pending", "Processing"] } }, { $set: { status: "Successful", razorpayPaymentId: paymentId, razorpaySignature: signature, transactionReference: paymentId, verifiedAt: paidAt, paidAt, receiptNumber: receipt(payment) } }, { new: true });
  if (!updated) return Payment.findById(payment._id);
  if (updated.purpose === "SCHOOL_SUBSCRIPTION") {
    const s = await SchoolSubscription.findOne({ schoolName: updated.schoolName });
    if (s) {
      const billing = updated.metadata?.billingPeriod;
      const start = billing?.start ? new Date(billing.start) : new Date(s.nextBillingDate || s.billingStartDate || paidAt);
      const end = billing?.end ? new Date(billing.end) : addMonths(start, 1);
      s.nextBillingDate = end;
      s.currentBillingPeriod = { start, end };
      s.status = "Active";
      await s.save();
    }
  }

  broadcastEvent(req, "payment:updated", { paymentId: updated._id, schoolName: updated.schoolName, purpose: updated.purpose });
  broadcastEvent(req, "subscription:updated", { schoolName: updated.schoolName });
...
exports.listPayments = async (req, res) => { try { const u = await User.findById(req.user.id); let q = {}; if (u.role === "superadmin") { const purpose = req.query.purpose; if (purpose && purpose !== "All") { q = { purpose }; } else { q = {}; } } else if (u.role === "admin") { q = { schoolName: u.schoolName }; } else if (u.role === "teacher") { q = { receiver: u._id, purpose: "TEACHER_SALARY" }; } else { q = { payer: u._id }; } res.json(await Payment.find(q).populate("payer receiver", "name email role").sort({ createdAt: -1 }).limit(100)); } catch (e) { fail(res, e); } };
...
exports.setTeacherCompensation = async (req, res) => { try { const salary = Number(req.body.salary), paymentCycle = req.body.paymentCycle || "MONTHLY", validityDays = Number(req.body.validityDays || 30), dueDate = req.body.dueDate ? new Date(req.body.dueDate) : null; const teacher = await User.findOne({ _id: req.params.teacherId, role: "teacher", schoolName: req.user.schoolName }); if (!teacher || isNaN(salary) || salary < 0 || !["MONTHLY", "WEEKLY", "ONE_TIME"].includes(paymentCycle) || (dueDate && Number.isNaN(+dueDate))) throw invalid("Valid teacher salary, payment cycle and due date are required"); const old = await TeacherCompensation.findOne({ teacher: teacher._id, schoolName: req.user.schoolName }).lean(); const pay = await TeacherCompensation.findOneAndUpdate({ teacher: teacher._id, schoolName: req.user.schoolName }, { $set: { salary, paymentCycle, validityDays, dueDate, active: req.body.active !== false, updatedBy: req.user.id } }, { new: true, upsert: true }); await audit(req, "TEACHER_SALARY_CHANGED", old, pay.toObject()); res.json(pay); } catch (e) { fail(res, e); } };
exports.setSubscription = async (req, res) => {
  try {
    ...
    if (!schoolName || isNaN(monthlyFee) || monthlyFee < 0 || !Number.isInteger(gracePeriodDays) || gracePeriodDays < 0 || gracePeriodDays > 90) {
...
exports.dashboard = async (req, res) => { try {
  ...
  const q = u.role === "superadmin" ? {} : u.role === "admin" ? { schoolName: u.schoolName } : u.role === "teacher" ? { receiver: u._id, purpose: "TEACHER_SALARY" } : { payer: u._id, purpose: "STUDENT_SCHOOL_FEE" };
  ...
  if (u.role === "superadmin") {
    const subPayments = successful.filter(p => p.purpose === "SCHOOL_SUBSCRIPTION" || p.receiverRole === "superadmin");
    platformRevenue = subPayments.reduce((sum, p) => sum + p.amount, 0);
    totalSystemVolume = successful.reduce((sum, p) => sum + p.amount, 0);
    totalReceived = platformRevenue;
    monthlyReceived = monthly.filter(p => p.purpose === "SCHOOL_SUBSCRIPTION" || p.receiverRole === "superadmin").reduce((sum, p) => sum + p.amount, 0);
  ...
  const base = { totalReceived, monthlyReceived, totalPaid, subscriptionPaid, platformRevenue, totalSystemVolume, pendingPayments: payments.filter(p => ["Pending", "Processing", "PendingVerification"].includes(p.status)).length, paymentCount: payments.length, recentPayments: payments.slice(0, 10) };
  if (u.role === "superadmin") {
    const subscriptions = await SchoolSubscription.find();
    const billed = await Promise.all(subscriptions.map(async sub => {
      try {
        return { sub, state: await subscriptionState(sub) };
      } catch (e) {
        return { sub, state: { status: sub.status || "Payment Due", amountDue: sub.monthlyFee || 0, paymentRequired: true, currentBillingPeriod: { start: sub.billingStartDate, end: sub.nextBillingDate }, remainingDays: 0 } };
      }
    }));
    ...
    for (const p of successful) {
      ...
      schoolBreakdownMap[sName].totalVolume += p.amount;
      if (p.purpose === "SCHOOL_SUBSCRIPTION" || p.receiverRole === "superadmin") {
        schoolBreakdownMap[sName].superAdminRevenue += p.amount;
      } else if (p.purpose === "STUDENT_SCHOOL_FEE") {
        schoolBreakdownMap[sName].totalStudentFees += p.amount;
      } else if (p.purpose === "TEACHER_SALARY") {
        schoolBreakdownMap[sName].totalTeacherSalaries += p.amount;
      }
    }

    return res.json({ ...base, totalRevenue: platformRevenue, platformRevenue, totalSystemVolume, monthlyRevenue: base.monthlyReceived, activeSchools: billed.filter(x => x.state.status === "Active").length, freeSchools: billed.filter(x => x.state.status === "Free").length, dueSchools: billed.filter(x => ["Due Soon", "Payment Due", "Grace Period"].includes(x.state.status)).length, suspendedSchools: billed.filter(x => x.state.status === "Suspended").length, upcomingPayments: billed.filter(x => x.state.paymentRequired).map(x => ({ schoolName: x.sub.schoolName, amount: x.state.amountDue, dueDate: x.state.currentBillingPeriod.start, status: x.state.status })), freePeriodsEndingSoon, schoolRevenueBreakdown: Object.values(schoolBreakdownMap) });
  }
  if (u.role === "admin") {
